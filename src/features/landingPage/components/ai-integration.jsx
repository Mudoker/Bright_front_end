...
export const Card = ({ className, children }) => {
    return (
        <div
            className={cn(
                'group mx-auto w-full max-w-sm rounded-xl border border-[rgba(255,255,255,0.10)] bg-[rgba(40,40,40,0.70)] p-8 shadow-[2px_4px_16px_0px_rgba(248,248,248,0.06)_inset]',
                className
            )}
        >
            {children}
...
export const CardDescription = ({ children, className }) => {
    return (
        <p
            className={cn(
                'max-w-sm text-sm font-normal text-neutral-400',
                className
            )}
        >
            {children}
